...
int main(int argc, char *argv[])
{
   ...
   // pid_t pid;
    char *message;
   // int n;
    printf("(process 3): program starting\n");
  ...
  //  char buffer[256];
   ...
   // int userinput;
    ...
    while(times == 1){
        ...
        puts("(Process 3) trying to receive\n");
        receivingMessage.message_type = 3;
        receiveMessage(msgId, &receivingMessage, BLOCK);

        puts("(Process 3) received message\n");


        if(receivingMessage.data == -1){

            sendingMessage.data = -1;
            sendingMessage.message_type = (long) 1;
