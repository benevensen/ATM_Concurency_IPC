...
const int clientMessageType = 1;
const int serverMessageType = 2;
...
int getShmId(size_t size){
    ...
    if ((shmid_1 = shmget(shmKey, size , 0666 | IPC_CREAT)) == -1)
    {
...
int SemaphoreWaitNthSemInSetWithBlock(int semid, int iMayBlock, int n)
{
    int flag;
...
int SemaphoreSignalNthSemInSetFromBlock(int semid, int n)
{
    struct sembuf sbOperation;
    sbOperation.sem_num = n;
    sbOperation.sem_op = +2;
